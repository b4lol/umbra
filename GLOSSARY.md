...
- **View-Once Media:** The media protection in which, the moment the dialog closes or the finger is lifted after the user opens a photo, the $EFK$ key is destroyed and wiped out of RAM, making a second opening mathematically impossible.
- **Crypto-Shredding:** The technique whereby, when the 7/10-day period expires, the data is not sought out and deleted; instead, the unique $EFK$ key that encrypted it is destroyed, turning the data into irrecoverable random noise.
- **Anti-Clock Tampering:** Time verification via Tor Consensus Time and the `CLOCK_MONOTONIC_RAW` differential, used to prevent the user from rolling the device clock back to extend the 7/10-day retention period.
...
- **Typestate Pattern:** The design pattern that makes invalid state transitions impossible at compile time through the type system (*Make Illegal States Unrepresentable*) and reduces state-machine logic errors to zero.
