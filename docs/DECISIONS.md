...
- **Residual: `umbra-nym-cli`'s own independent lockfile carries its own unresolved advisories.** Because this crate has a fully separate `Cargo.lock` (the whole point of this ADR's isolation), it also needs its own, separate `cargo audit` pass — the main workspace's audit (Task 1 of this plan) never touches it. Run directly against `crates/umbra-nym-cli/Cargo.lock`, `cargo audit` reports 4 hard vulnerabilities, all transitive from `nym-sdk`'s own pinned dependency tree, none touching the `blake3`/`arrayref` pair addressed above: `h2` 0.3.27 (RUSTSEC-2026-0258, unbounded empty DATA frames / DoS), and `rustls-webpki` 0.101.7 (RUSTSEC-2026-0098, RUSTSEC-2026-0099, RUSTSEC-2026-0104, certificate-validation bugs) — plus 4 unmaintained-crate warnings (`bincode`, `derivative`, `proc-macro-error2`, `rustls-pemfile`). Separately, `rsa` 0.9.10 is present in the same tree and carries RUSTSEC-2023-0071 (the "Marvin Attack" timing side-channel, no upstream fix exists for any `rsa` version); a real, well-known, ecosystem-wide, currently-unpatchable issue applicable to any consumer of the `rsa` crate, this one included. None of this is a defect introduced by Umbra's own code: these are `nym-sdk`'s own transitive dependency choices, and no clean fix is available within this project's scope (patching `h2`/`rustls-webpki` independently would mean fighting `nym-sdk`'s own pinned tree; `rsa`'s advisory has no patched version to move to at all). Tracked as an upstream-dependency residual to revisit at `nym-sdk`'s next version bump, not something this plan attempts to "fix" directly. All five advisories (rsa, h2, and the three rustls-webpki entries) are therefore listed as explicit, individually justified ignores in `.cargo/audit.toml`, so `just audit-nym` gates on NEW advisories instead of re-failing on this known, accepted set.
...
- **Addendum (2026-09-10, TODO B.2.1/B.2.2/B.2.3 landed):** the three ruled follow-ups shipped as one increment. Roster bootstrapping was closed with typed application-level payloads (tag `0x00` user text / `0x01` roster sync, `crates/umbra-group/src/roster_sync.rs`) riding the group's own AEAD channel — no MLS protocol change, no leaf-credential surgery (the rejected alternative), and a strictly-newer-epoch staleness guard (`GroupRoster::last_sync_epoch`) because MLS's skipped-key retention makes out-of-epoch application-message delivery reachable over unordered transports. `umbra group create` gained `--self-name` because a full snapshot must name every member including its sender, and a roster deliberately never lists the local member. Member removal (`umbra group remove`) and manual key rotation (`umbra group rotate`, `self_update`) landed with the same durable-before-delivery semantics as `add_member`; removal deliberately has NO ACL (the single-cell trust model is co-equal — any member can already add members or withhold deliveries, so removal power adds no new capability class).

---

## ADR-034: `umbra-hwkey`'s PKCS#11 Module Loading — A Scoped Runtime Deviation from ADR-011

- **Status:** Accepted (Documented Deviation / Exception to ADR-011)
- **Rationale:** ADR-011 bans C and C++ from the project's own code and direct dependencies. PKCS#11 (the hardware-security-key integration surface `crates/umbra-hwkey` implements, TODO B.3) is by definition a C ABI standard — no pure-Rust PKCS#11 TOKEN implementation exists to link against instead, only pure-Rust CLIENT bindings (`cryptoki`) that load a third-party C module (SoftHSM2, OpenSC, or a hardware vendor's own PKCS#11 library) via `dlopen` at runtime. Rejecting this would mean no PKCS#11-based hardware-key support at all — this ADR accepts the deviation rather than dropping the feature, the same trade-off ADR-028 made for the Tor stack's C-bearing transitive dependencies.
- **Decision:**
  - `cryptoki`'s pure-Rust FFI bindings are the only sanctioned way to speak PKCS#11 — no direct `unsafe` FFI of our own to a PKCS#11 module; `umbra-hwkey` itself stays `#![forbid(unsafe_code)]`.
  - The loaded module is a RUNTIME `dlopen`, not a build-time link — `cryptoki` itself needs no C dev headers to build (confirmed: no `links` key, no system dev-package requirement), unlike ADR-028's Tor-stack dependencies.
  - `module_path` (the path to the `.so` to load) is a caller-supplied trust boundary: whoever calls `generate_hmac_key`/`challenge_response` controls what code gets mapped into this process. This crate does no signature or provenance verification of the module itself — that responsibility belongs to whichever future increment decides how `module_path` is sourced (a fixed, hardcoded path to a known-good system module is the safest option; accepting an arbitrary user-supplied path is not, and should be a deliberate, separately-reviewed decision if it's ever proposed).
  - This deviation is scoped to `crates/umbra-hwkey` only — no other Umbra crate may load a PKCS#11 (or other third-party C) module.
- **Consequence:** Hardware-security-key support becomes possible without dropping the "no C" policy for the rest of the codebase; the residual risk (an untrusted `module_path` loading arbitrary code) is documented rather than hidden, and is a landing precondition for whichever increment decides how a production caller sources that path — that increment must also account for this crate's `dlopen` under this project's Landlock/Seccomp sandboxing model, which does not yet grant any exception for it.
  - **Addendum (2026-09-14, CLI hardware-key wiring increment):** the deliberate decision ADR-034 asked for has been made — `crates/umbra-cli`'s new `--hw-module PATH` global flag accepts an ARBITRARY user-supplied path with no signature/provenance verification, accepting the risk explicitly rather than restricting to a fixed, hardcoded module path. Rationale: this is a local-only, argv-controlled input (the same trust level as `--keystore PATH`/`--passphrase-file PATH` already have — a local attacker who controls CLI arguments already controls far more than this), and a hardcoded path would make the flag unusable against any non-SoftHSM2 real hardware token (a vendor's own PKCS#11 module lives at a vendor- and distro-specific path this project cannot enumerate in advance). The residual risk (a malicious `--hw-module` value `dlopen`s attacker-controlled code into a process that has already run `mlockall`) is accepted as bounded by the existing local-argv trust model, not eliminated. ADR-034's second precondition — accounting for this `dlopen` under Landlock/Seccomp — is satisfied in fact by construction: every `load_identity`/`init_with` call site runs before `harden_sandbox()`, never after (verified across all six `load_identity` call sites in `cli.rs`), so the sandbox is never active during the `dlopen`. This ordering is load-bearing and must be preserved by any future refactor.
