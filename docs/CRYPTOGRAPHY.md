...
- **Logarithmic Scaling ($O(\log N)$):** When a member is added or removed, only the tree path is updated instead of re-distributing keys to the entire group from scratch — inherited unmodified from OpenMLS (removal landed 2026-09-10, TODO B.2.2: `umbra group remove` commits a Remove proposal; on-demand key rotation landed alongside, TODO B.2.3: `umbra group rotate` commits a `self_update`).
...
- **Honest scope — what the group engine does NOT cover (tracked, not hidden; see TODO B.2 for the full list):** sub-groups, large-group scalability testing, a full post-quantum signature ciphersuite, inbound group-frame handling over the mesh/Nym transports (Tor-only for now), an ACL for membership changes (any member is co-equal — the deliberate single-cell trust model), and automatic/periodic key rotation (rotation is manual-only, TODO B.2.3). Member removal and manual rotation themselves LANDED 2026-09-10 (TODO B.2.2/B.2.3). Roster bootstrapping for a newly joined member landed 2026-09-10 (TODO B.2.1): the adding member fans out a full roster snapshot as a typed MLS application message (`crates/umbra-group/src/roster_sync.rs`), so a `Welcome`-joined member can send as soon as the sync arrives. See `THREAT_MODEL.md` and `TODO.md` B.2 for the full, itemized residual list.
