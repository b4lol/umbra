    ...
    match kind {
        ...
        TransportKind::Group => Profile {
            ...
            maturity_note: "single-cell increment validated at 3-party scale; \
                            member removal and on-demand key rotation are both \
                            implemented (TODO B.2.2/B.2.3) — no automatic/ \
                            periodic rotation policy",
            guidance: "a member who joins via an inbound Welcome can send once \
                       RosterSync (TODO B.2.1) delivers the group's roster to \
                       them, which may take one round trip after joining — \
                       group frames are Tor-only",
            doc_ref: PROFILES_DOC,
        },
    }
}
...
mod tests {
    ...
    fn group_content_rating_carries_the_partial_pq_caveat() {
        ...
        assert!(group.guidance.contains("RosterSync"));
    }
}
