...
//! (TODO B.2). Currently `create`/`export-keypackage`/`add`; later
//! tasks in the same plan add invite/join/send/receive operations to
//! this same enum.
//!
//! # `add`'s transport scope (ruled, binding — see the SDD ledger's
//! "Rulings before Task 8 dispatch" entry)
//!
//! `umbra-group` itself (`add_member`) is transport-agnostic — it never
//! dials anything directly, only calls a caller-supplied `connect`
//! closure. THIS module supplies that closure, and its real,
//! functioning implementation covers ONLY [`PeerTransportAddress::Onion`]
//! (via a bootstrapped [`umbra_net::tor::TorTransport`], `#[cfg(feature
//! = "tor")]`, mirroring `tor_send.rs`'s own bootstrap pattern but with
//! NO PQXDH handshake — group frames carry their own MLS-level
//! encryption, so the stream is handed straight to `deliver_to_members`
//! once opened). [`PeerTransportAddress::Mesh`]/[`PeerTransportAddress::Nym`]
//! (and a peer record with no address on file at all, which
//! `peer_lookup` itself already reports as `None`) return a clear,
//! explicit [`umbra_group::GroupError`] rather than attempting real
//! mesh/Nym dialing — an intentional, ruled scope bound for this task
//! (membership addition + a delivery ATTEMPT, not full multi-transport
//! CLI maturity), not an oversight. Building without the `tor` feature
//! also returns a clear error for onion addresses (mirrors
//! `Command::Send`'s existing `#[cfg(not(feature = "tor"))]` fallback
//! in `cli.rs`), rather than a confusing "not yet implemented" message
//! that would misleadingly suggest the transport itself is unbuilt
//! everywhere.
