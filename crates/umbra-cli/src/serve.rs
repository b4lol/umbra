...
//! flow (`mesh_serve.rs`) and `umbra-nym-cli`'s bridge each have their
//! own, separate accept path, but reuse this crate's transport-agnostic
//! group-frame parsing/processing (`crate::group_inbound`, TODO B.2.4)
//! rather than duplicating it — group receipt is no longer Tor-only.
...
// TODO B.2.4: the group-cell inbound plumbing (frame parsing,
// `GroupInboundContext`, `InboundEvent`) moved to `crate::group_inbound`
// so `mesh_serve.rs` (built with `--features mesh`, no `tor`) and
// `umbra-nym-cli` (which builds this crate with NEITHER feature) can
// reach it too. Re-exported here so every existing caller of this
// module (`tui.rs`, `cli.rs`, `group.rs`, and this module's own tests)
// keeps working unchanged.
#[cfg(test)]
pub(crate) use crate::group_inbound::MAX_GROUP_FRAME_FIELD;
pub use crate::group_inbound::{
    GroupInboundContext, InboundEvent, group_context_from_keystore, handle_group_frame,
    prepare_group_paths, read_group_frame,
};
...
pub fn tor_base_from_keystore(keystore: &Path) -> Result<PathBuf, CliError> {
    Ok(crate::group_inbound::keystore_parent(keystore)?.join("tor"))
}
