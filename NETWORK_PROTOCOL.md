...
- **Umbra's own cover traffic is unchanged and additional:** the SDK's mix-network cover packets described above operate *between* gateways, on the outside of the message. Inside it, the bytes `send_text_stream` produces still carry Umbra's own `DUMMY_COVER` (`0x04`) frames, up to `MAX_COVER_PER_SEND` per burst, exactly as on the Tor and mesh transports — see `SPECIFICATION.md` §1 ("1024-Byte Binary Packet Layout"). The two layers are independent; neither substitutes for the other.
- **Identity persistence differs by direction.** `serve-nym` uses on-disk persistent storage (`StoragePaths` rooted at its `--nym-config` directory) so its Nym address survives process restarts, matching the `.onion` identity model. `send-nym` is the opposite: it builds an EPHEMERAL, in-memory client (`MixnetClientBuilder::new_ephemeral()`) and writes no transport identity material to disk at all. It can afford to, because PQXDH lets a sender complete and transmit using only the recipient's asynchronously-known prekey bundle — there is no reply at the Nym transport layer, so a sender needs no stable, reachable address. Consequently `send-nym` has no config directory and exposes no flag to give it one.
