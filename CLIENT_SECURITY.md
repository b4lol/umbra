...
- Using the Linux kernel's `Landlock` security module, the Umbra process's read and write access to the filesystem is reduced to absolute zero — with two documented exceptions: the controlling terminal `/dev/tty` (ReadFile+WriteFile+IoctlDev: crossterm raw-mode termios ioctls and redirected-stdin reopen need them) and, for flows hosting an onion service, the caller-supplied Tor storage directory (narrowed grant: regular files/dirs only — no Execute, device/socket/fifo creation, or IoctlDev; Arti's native keystore there keeps the `.onion` identity persistent — TODO A.2). The zero-access default and the exception mechanism are verified in `sandbox_landlock.rs`; the tty rule itself is environment-dependent (headless CI has no controlling terminal) (CLIENT sandbox implementation: `umbra-cli::sandbox`).
...
### D. CPU Register Zeroing (`zero-call-used-regs`) — BLOCKED UPSTREAM
- ADR-025 mandates `-Z zero-call-used-regs=all`; the flag was REMOVED from rustc (nightly 1.100.0, neither `-C` nor `-Z` accepted) without ever stabilizing — clang's `-fzero-call-used-regs` LLVM feature has no surviving rustc front-door. Tracked as blocked in TODO A.4; no equivalent codegen switch exists in current rustc.
