...
- **CLI hardware-key wiring** (TODO B.3, third increment of the FIDO2/hardware-key item): `umbra init --hw-module/--hw-pin-file/--hw-key-label` creates a hardware-key-gated keystore; every command sharing the CLI's `load_identity` helper (`send`, `recv`, `serve-mesh`, `fingerprint`, `export-pairing`, `pair`, `group`) can unlock one, auto-detected from the file itself. `umbra-cli` now depends on `umbra-hwkey` in production code. `umbra serve`/`tui` and `umbra-nym-cli` remain passphrase-only (tracked follow-up). Also closes the CI gap blocking `umbra-hwkey`'s and `umbra_cli::keystore`'s hermetic SoftHSM2 tests (portable module-path resolution + a CI install step).
